...
class MeanShiftStep(torch.nn.Module):
    ...
    def mean_shift_step(points, reference=None, bandwidth=1, kernel='gaussian', use_keops=True):
        ...
        if kernel == MeanShiftStep.GAUSSIAN_KERNEL:
            factor = points.new([-1 / (2 * bandwidth ** 2)])  # uses the shape, device, dtype of points with new data
...
class MeanShift(torch.nn.Module):
    ...
    def forward(self, points):
        ...
        Points that will be shifted. Shape should be arbitrary batch dimensions (B) followed by number of points (N) and
        ...
        # points should have shape B N E
        current = points
        for i in tqdm(range(self.n_iter), desc="Mean Shift Iteration", leave=False):
            if self.blurring:
                current = self.step(current)
            else:
                current = self.step(current, points)
            tqdm.write("Done with iteration {}".format(i))
        return current
